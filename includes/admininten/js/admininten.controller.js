(function() {
    ...
    angular.module('Controllers')
        .controller('adminintenController', adminintenController);

    adminintenController.$injet = ['LxDialogService', 'LxDatePickerService', '$http', '$httpParamSerializerJQLike', 
                                        'SessionService' ];

    function adminintenController(LxDialogService, LxDatePickerService, $http, $httpParamSerializerJQLike, 
                                        SessionService ) {
        //var vm = this;
        var vm = this;
...
//        vm.copia = vm.campos; 
        ...
        "recaptcha_response": token,
            ...
            $http({
                ...
                url: '/siiFCA/api/login_admin.php'
            })
        ...
        function cerrarSesion() {
            ...
            $http({
                ...
                headers: { 'Content-Type': 'application/x-www-form-urlencoded',
                            'Data-Type': 'html' },
                url: '/siiFCA/api/logout_admin.php'
            })
            .then( function ok(resp) {
                vm.showLinearProgress = false;
                vm.enviado = false;
...
*/
        ...
        function enviarDatosRegistro( token ) {        // recupera los datos y los envÃ­a al API / BD
            ...
            var datosEnviar = { "numero"    : vm.campos.numero,
                                ...
                                "civil"     : (vm.campos.civil !== undefined) ? vm.campos.civil.name : "O+",
            ...
            $http({
                ...
                url: '/siiFCA/api/registro_admin.php'
            })
            .then( function ok(resp) {
                vm.showLinearProgress = false;
                vm.enviado = false;
        ...
        function sendRecuperaPassw( token ) {
            var datosEnviar = { "numero"    : vm.contra.numero,
            ...
            $http({
                ...
                url: '/siiFCA/api/retpassw_admin.php'
            })
                .then(function ok(resp) {
                    vm.showLinearProgress2 = false;
                    vm.enviado = false;
        ...
        function verificaPassword() {
            ...
            if (noError) {
                ...
                var datosEnviar = { "administrativo" : vm.tpassw,
                                    "registrar"      : "verifica"
                };
                //console.log($httpParamSerializerJQLike(datosEnviar));
                ...
                $http({
                    ...
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                                'Data-Type': 'html' },
                    url: '/siiFCA/api/verifica_admin.php'
                })
                .then( function ok(resp) {
                    vm.showLinearProgress3 = false;
                    vm.enviado = false;
        ...
        function sendActualizarDatos( token ) {
            ...
            $http({
                ...
                data: $httpParamSerializerJQLike(datosEnviar),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Data-Type': 'html' },
                url: '/siiFCA/api/actualiza_perfil_admin.php'
            })
                .then(function ok(resp) {
                    vm.showLinearProgress = false;
                    vm.enviado = false;
