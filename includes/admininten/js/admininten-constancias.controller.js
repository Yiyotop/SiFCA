(function() {
    ...
    angular.module('Controllers')
        .controller('adminintenConstanciasController', adminintenConstanciasController);

    adminintenConstanciasController.$injet = ['LxDialogService', '$http', '$httpParamSerializerJQLike', 'LxDataTableService',
                'SessionService', '$filter', '$scope', 'LxNotificationService'];

    function adminintenConstanciasController(LxDialogService, $http, $httpParamSerializerJQLike, LxDataTableService,
                            SessionService, $filter, $scope, LxNotificationService) {
        //var vm = this;
        var vm = this;
        ...
        if (vm.adminValidado) { // si inicio sesión normal
            leerConstancias();
        }
        ...
        function leerConstancias() {        // recupera los datos y los enví­a al API / BD
            ...
            if (noError) {
                ...
                $http({
                    ...
                    url: '/siiFCA/api/leer_constancias_advo.php'
                })
        ...
        function verConstancia() {        // recupera los datos y los enví­a al API / BD
            ...
            if (noError) {
                ...
                $http({
                    ...
                    data: $httpParamSerializerJQLike(datosEnviar),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded','Data-Type': 'html' },
                    url: '/siiFCA/api/ver_constancia_advo.php'
                })
                .then( function ok(resp) {
                    vm.showLinearProgress = false;
                    vm.enviado = false;
